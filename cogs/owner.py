...
class owner(commands.Cog):
	...
	async def _decline(self, ctx, ID, *,reason):
		...
		await user.send(f"We have Declined your video because {reason}! With {ID} as its id. You can DM {ctx.author} in our support server if you want to dispute the problem!")
		await log.send(
			user.mention,
			embed=discord.Embed(
