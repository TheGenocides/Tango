...
class owner(commands.Cog):
    ...
    async def _verify(self, ctx, ID):
        ...
        if not user:
            await ctx.send(
                "Oh no cant find the user idk why. ask my dev to fix this error!"
            )
            return
        ...
        await con.close()
        await ctx.send(
            f"verified video with `{ID}` as its ID. I will dm the user to announce the news!"
        )
        await user.send(
            f"We have verified your video! with `{ID}` has its ID! Thanks for using TangoBot!"
        )
        await log.send(
            user.mention,
            embed=discord.Embed(
    ...
    async def _decline(self, ctx, ID, *, reason):
        ...
        if not data:
            await ctx.send(
                "wrong id or the video already got decline/delete permanently! :/"
            )
            return
        ...
        await con.close()
        await ctx.send(
            f"Declined video with `{ID}` as its ID. I will dm the user to announce the news"
        )
        await user.send(
            f"We have Declined your video because {reason}! With {ID} as its id. You can DM {ctx.author} in our support server if you want to dispute the problem!"
        )
        await log.send(
            user.mention,
            embed=discord.Embed(
            ...
            .set_author(
                name=f"Declined By {ctx.author}", icon_url=ctx.author.avatar.url
            ),
        )
    ...
    @commands.command(
        "reboot", description="Restart the entire bot", aliases=["restart"]
    )
    @commands.is_owner()
    async def _reboot(self, ctx, flags=None):
        if not flags:
            try:
                if self.bot.processing_commands > 1:
                    await ctx.send(
                        "I cant reboot right now because someone still using my command!"
                    )
                    return

                await ctx.send(
                    f"Rebooting: Im Online ;) {ctx.author.mention} in 3 seconds"
                )
        ...
        elif flags.lower() in ["-force", "--force"]:
            try:
                await ctx.send(
                    f"Rebooting: Im Online ;) {ctx.author.mention} in 3 seconds"
                )
