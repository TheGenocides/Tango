...
class fun(commands.Cog):
    ...
    @commands.command(
        "bar",
        description="Percentage the number given and turn it into bar similar to exp bar",
    )
    ...
    async def _lyric(self, ctx, *, name):
        try:
            ...
            msg = await ctx.send(
                embed=discord.Embed(
                    description="**Select The Language!**", color=discord.Color.orange()
                ),
                components=[
                    ActionRow(
                        Button(
                        ...
                        Button(
                            style=ButtonStyle.green, label="Kanji", custom_id="kanji"
                        ),
                        Button(
                            style=ButtonStyle.green, label="Romaji", custom_id="romaji"
                        ),
                        Button(
                            style=ButtonStyle.red, label="Cancel", custom_id="abort"
                        ),
                    )
                ],
            )
    ...
    async def _search(self, ctx, *, name):
        try:
            ...
            if anime[16] and not ctx.channel.is_nsfw():
                await ctx.send(
                    "Looks like this anime is nsfw :/ make sure to use this command in nsfw channels!"
                )
                return
            ...
            msg = await ctx.send(
                embed=discord.Embed(
                    description="**Select A Category!**", color=discord.Color.orange()
                ),
                components=[
                    ActionRow(
                        Button(
            ...
            async def _episodes_Button(inter):
                await inter.reply(
                    type=7,
                    embed=discord.Embed(
                        title=anime[0], url=anime[1], color=discord.Color.orange()
                    )
                    .set_thumbnail(url=anime[3])
                    .add_field(name="📺 Episodes", value=anime[4])
                    .add_field(name="🗃️ Genres", value=",\n".join(anime[10]))
                    .add_field(
                        name="<a:typingstatus:873477367522263081> Status",
                        value=anime[12],
                    )
                    .add_field(name="🌠 Teaser", value=f"[Click Me!]({anime[6]})")
                    .add_field(name="🎬 Opening", value=anime[7])
                    .add_field(name="🎬 Ending", value=anime[8]),
                    components=[
                        ActionRow(
                            Button(
                                style=ButtonStyle.grey,
            ...
            async def _mis_button(inter):
                await inter.reply(
                    ...
                    .set_thumbnail(url=anime[3])
                    .add_field(name="🏅 Ranked", value=anime[9])
                    .add_field(name="🏅 Rating", value=anime[13])
                    .add_field(name="⌛ Aired", value=anime[5])
                    .add_field(
                        name="<:Offline:873411447303057489> Type", value=anime[11]
                    )
                    .add_field(name="🤩 Popularity", value=anime[14])
                    .add_field(
                        name="🎥 Producers",
                        value=",\n".join(anime[15]),
                    ),
            ...
            await msg.edit(
                content=f"No anime called `{name}`. Make sure to type it properly"
            )
    ...
    async def _watch(self, ctx, player=3, time=100):
        ...
        link = await self.game.create_link(
            ctx.author.voice.channel.id, "youtube", max_age=time, max_uses=player
        )
        await ctx.send(
    ...
    async def _chess(self, ctx, player=3, time=100):
        ...
        link = await self.game.create_link(
            ctx.author.voice.channel.id, "chess", max_age=time, max_uses=player
        )
        await ctx.send(
            f"**Chess Together!**\n**`Host > Click the Link`**\n**`Player > Click the play button, you do need to wait till the host click the blue link!`**\n**`Spectator > Click the spectate button`**\nlink ---> {link}"
        )
    ...
    @commands.command(
        "fishing",
        aliases=["fish"],
        description="play a game of fishing with your friend!",
    )
    async def _fishing(self, ctx, player=3, time=100):
        ...
        link = await self.game.create_link(
            ctx.author.voice.channel.id, "betrayal", max_age=time, max_uses=player
        )
        await ctx.send(
            f"**Fishing Together!**\n**`Host > Click the Link`**\n**`Player > Click the play button, you do need to wait till the host click the blue link!`**\n**`Spectator > Click the spectate button`**\nlink ---> {link}"
        )
    ...
    @commands.command(
        "betrayal",
        aliases=["betray"],
        description="play a game of betrayal with your friend!",
    )
    ...
    async def _poker(self, ctx, player=3, time=100):
        ...
        link = await self.game.create_link(
            ctx.author.voice.channel.id, "poker", max_age=time, max_uses=player
        )
        await ctx.send(
            f"**Poker Together!**\n**`Host > Click the Link`**\n**`Player > Click the play button, you do need to wait till the host click the blue link!`**\n**`Spectator > Click the spectate button`**\nlink ---> {link}"
        )
