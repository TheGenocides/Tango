...
async def find_videos(condition):
    ...
    await cur.execute(
        "SELECT * FROM video WHERE deleted = ? AND verified = ?", ("n", "y")
    )
    data = await cur.fetchall()
    ...
    full_data = [
        ...
        for x in data
        if condition in x[1]
        or condition in x[1].split(" ")
        or condition.lower() in x[1]
        or condition.lower() in x[1].split(" ")
        or condition.upper() in x[1]
        or condition.upper() in x[1].split(" ")
        or condition in x[1].lower()
        or condition.lower() in x[1].lower()
        or alt_condition in x[1]
        or alt_condition.lower() in x[1]
        or alt_condition in x[1].lower()
        or alt_condition.lower() in x[1].lower()
        or alt_condition in x[1][:3]
    ]
    await cur.close()
    await con.close()
    return full_data
...
async def find_in_video(condition, deleted: bool, *, mode="one", verified=None):
    ...
    if verified is None:
        ...
        cur = await cursor(con)
        await cur.execute(
            "SELECT * FROM video WHERE member_id = ? AND deleted = ?",
            (condition, deleted),
        )
        ...
        cur = await cursor(con)
        await cur.execute(
            "SELECT * FROM video WHERE member_id = ? AND deleted = ? AND verified = ?",
            (condition, deleted, verified),
        )
...
async def comments(bot, commenter_id: int, video_id: int, content: str):
    ...
    cur = await cursor(con)
    await cur.execute(
        "CREATE TABLE IF NOT EXISTS comments(commenter_id int, video_id int, content text, comment_id, date)"
    )
    try:
        _date = int(time.time())
        await cur.execute(
            "INSERT OR IGNORE INTO comments(commenter_id, video_id, content, comment_id, date) VALUES (?,?,?,?,?)",
            (commenter_id, video_id, content, comment_id, _date),
        )
        await con.commit()
        ...
        cur = await cursor(con)
        await cur.execute(
            "UPDATE video SET comments = comments + 1 WHERE ID = ?", (video_id,)
        )
...
def anime(name):
    ...
    data = (
        ...
        anime.aired,
        anime.teaser,
        random.choice(anime.opening_themes),
        random.choice(anime.ending_themes),
        anime.ranked,
        anime.genres,
        anime.type,
        anime.status,
